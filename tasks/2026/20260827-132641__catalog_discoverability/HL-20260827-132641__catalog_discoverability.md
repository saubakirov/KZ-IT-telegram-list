...
| Final-only text review discovers broad language problems late | Medium | High | Stabilize structure and completeness first, review all ready renders plus the changed-key manifest together, disposition Antigravity findings, and repeat the deterministic generation/check loop before the formal Reviewer rules on the final digest |
...
1. Phase A may prepare complete EN/RU/KK candidates, but acceptance waits until Antigravity has
   independently reviewed text quality and the formal Reviewer has verified the findings,
   dispositions, and ready rendered result bound to its final digest; machine output may propose
   wording, while the delegated autonomous pipeline supplies the verdict without fabricating a
   separate human chat approval;
