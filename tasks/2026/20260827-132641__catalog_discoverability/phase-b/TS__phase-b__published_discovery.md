...
- Generate `sitemap.xml` through the GitHub Pages-supported `jekyll-sitemap` plugin and restrict it
  to the intended three public catalog routes.
...
|------|--------|-------------|
| `_config.yml` | CREATE | Root-source Jekyll/site configuration and supported sitemap plugin |
...
**Budget:** 7 new files, 5 modifications, 12 implementation paths, estimated delta below 2500 LOC.
...
- [ ] `_config.yml` uses the canonical `url`/`baseurl`, supported `jekyll-sitemap` plugin, and excludes
...
The supported GitHub Pages plugin produces one sitemap that identifies only the three public catalog routes.

- [ ] `sitemap.xml` is generated by supported `jekyll-sitemap`, not maintained by hand.
...
- Current authenticated settings establish that root-source branch deployment is already configured;
  do not change Pages source or add a custom workflow. `_config.yml`, one layout, and supported plugins
  are the intended repository-owned surface.
- As of planning, GitHub Pages dependency 232 exposes Jekyll 3.10.0 and `jekyll-sitemap` 1.4.0.
  Pin `github-pages` compatibly and record resolved versions; do not guess a newer unsupported plugin.
