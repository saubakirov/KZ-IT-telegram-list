...
| TD-19 | The historical Phase C offline harness hardcodes `oldest_live == 2026-01-30` while the current catalog's oldest live verification date is `2026-08-27`, so its full `main()` fails even though the reusable link and command matrices pass | test-harness | Low | 🔴 OPEN | [RF catalog intake Phase A §6](tasks/2026/20260828-201343__catalog_intake_commands/phase-a/RF__phase-a__intake_engine.md#6-observations-out-of-scope-not-modified) |
| TD-20 | Four regression tests hard-code the reviewed pre-apply catalog snapshot, so the full suite remains non-green after the exact authorized 20-row Phase B apply even though all 41 applicable invariants pass and no functional failure or error exists | test-contract | Medium | 🔴 OPEN | [REVIEW catalog intake Phase B §5](tasks/2026/20260828-201343__catalog_intake_commands/phase-b/REVIEW__phase-b__candidate_integration.md#5-tech-debt-collected) |
| TD-21 | The retained intake stage contains only controlled output files while apply preflight requires the validation surface under the stage root, forcing a separate deterministic manifest-bound materialization step before apply | intake-ux | Low | 🔴 OPEN | [REVIEW catalog intake Phase B §5](tasks/2026/20260828-201343__catalog_intake_commands/phase-b/REVIEW__phase-b__candidate_integration.md#5-tech-debt-collected) |
